...
<!-- ARM: compose accepts a scalar existing value as the top level input -->
<!-- ARM: compose embeds existing values at multiple list and map depths -->
<!-- ARM: compose accepts repeated inclusion of the same child value -->
<!-- ARM: compose mixes raw portable leaves and constructed value leaves -->
<!-- ARM: compose rejects every non value struct -->
<!-- ARM: compose rejects runtime references outside and beside valid value leaves -->
<!-- ARM: compose rejects an existing value used as an object key -->
...
<!-- ARM: compose produces canonical bytes equal to fully expanded construction -->
<!-- ARM: compose produces a normalized term equal to fully expanded construction -->
<!-- ARM: compose normalizes numbers across a composition boundary -->
<!-- ARM: compose orders newly introduced keys by UTF-16 code units -->
<!-- ARM: compose accepts child objects whose own keys are already canonical -->
<!-- ARM: to term of a composed value contains no nested value structs -->
...
<!-- ARM: compose enforces max bytes on the complete composed result -->
<!-- ARM: compose offsets child depth by its nesting position and enforces max depth -->
<!-- ARM: compose counts nodes exactly including each repeated child occurrence -->
<!-- ARM: compose enforces maximum string object member and array element limits -->
<!-- ARM: compose rejects a child constructed under limits larger than the composing call -->
...
<!-- ARM: compose rejects an obviously malformed value representation through bounded checks -->
<!-- ARM: a composed value round trips through canonical encode and decode -->
<!-- ARM: a composed value is completely revalidated after crossing a process boundary -->
...
<!-- ARM: composing the cell request envelope visits each raw outer node once -->
<!-- ARM: composing the cell request envelope incorporates children without walking their subtrees -->
<!-- ARM: the composed cell request envelope is byte identical to full reconstruction -->
<!-- ARM: property composing a mixed tree equals constructing the expanded tree -->
